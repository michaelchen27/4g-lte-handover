...
//
  ...
  // Vector enbPosition (0,0,0);
  for (uint16_t i = 0; i < numberOfEnbs; i++)
    {
       if (i == 3){
        Vector enbPosition (distance * (i +1), distance -500, 0);
        enbPositionAlloc->Add (enbPosition);
       }
      else if (i %2 == 0){
        Vector enbPosition (distance * (i +1), distance+1000, 0);
        enbPositionAlloc->Add (enbPosition);
      }else{
        Vector enbPosition (distance * (i +1), distance, 0);
        enbPositionAlloc->Add (enbPosition);
      }
      // Vector enbPosition (distance * (i +1), distance+50, 0);
    }
  ...
  anim.UpdateNodeDescription(enbNodes.Get(2), "eNB 3");
  anim.UpdateNodeDescription(enbNodes.Get(3), "eNB 4");
  anim.UpdateNodeDescription(enbNodes.Get(4), "eNB 5");
  anim.UpdateNodeDescription(enbNodes.Get(5), "eNB 6");

  anim.UpdateNodeDescription(pgw, "PGW");
  anim.UpdateNodeColor(pgw, 0, 255, 0);

  anim.UpdateNodeDescription(remoteHost, "RemoteHost");
  anim.UpdateNodeColor(remoteHost, 255, 102, 0);
